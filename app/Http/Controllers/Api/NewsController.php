...
class NewsController extends Controller {
 public function index(Request $r){
  $q=NewsItem::latest('published_at');
  if(!$r->user()?->isRole(Role::Admin,Role::Supervisor)){
   $q->where('status',NewsStatus::Published)->where(fn($x)=>$x->whereNull('published_at')->orWhere('published_at','<=',now()));
  }
  return ['data'=>$q->get()];
 }
