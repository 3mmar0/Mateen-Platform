...
let collection, addDoc, deleteDoc, doc, getDoc, onSnapshot, query, orderBy, serverTimestamp, onAuthStateChanged;
...
async function ensureFirebase() {
  ...
  ({ collection, addDoc, deleteDoc, doc, getDoc, onSnapshot, query, orderBy, serverTimestamp } = firestore);
...
function mapApiNewsItem(n) {
  ...
  return {
    ...
    tag: n.tag || '📢 خبر',
    pinned: !!n.pinned,
    dateLabel: date ? new Date(date).toLocaleDateString('ar', { day: 'numeric', month: 'short' }) : '',
  };
}
  ...
  list.innerHTML = items.map(n => `
    <div style="display:flex;align-items:center;gap:12px;padding:12px 16px;border-bottom:1px solid #f0ebe0">
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:11.5px;color:#8a6a52;margin-top:2px">${n.tag || ''} • ${n.dateLabel || ''}</div>
      </div>
      ${n.pinned ? '<span style="font-size:12px;color:#c9a227">📌</span>' : ''}
      <button onclick="deleteAdminNews('${n.id}')"
...
async function loadAdminNewsApi() {
  try {
    ...
    renderAdminNewsList((res?.data || []).map(mapApiNewsItem));
  } catch (e) {
    console.error('loadAdminNews API error:', e);
  }
}
...
if (useApi()) {
  api.me().then(me => {
    ...
    if (!['admin', 'supervisor', 'teacher'].includes(role)) return;
    addNewsButton();
    loadAdminNewsApi();
  }).catch(() => {});
} else {
...
function loadAdminNews() {
  if (useApi()) { loadAdminNewsApi(); return; }
  ensureFirebase().then(() => {
  ...
  try {
    if (useApi()) {
      await api.news.create({
        title, body, status: 'published', published_at: new Date().toISOString(),
      });
      loadAdminNewsApi();
    } else {
      await ensureFirebase();
